...
def cancel_booking(ticket_id):
    ...
    conn.commit()
    print("Your ticket deleted")

def book_ticket_buying():
    name = input("Write your name(example: Nurdaulet): ")
    phone = int(input("Write your phone number(example: 87007007070): "))
    afisha_id = select_movie_table()
    seats = get_available_seats(afisha_id)
    davailable_seats(seats)
    place_id = int(input("Select seat ID(example: 1): "))
...
def export_to_excel(database):
    ...
    data_frame.to_excel("ticket.xlsx")
    conn.close()
export_to_excel('database.db')
